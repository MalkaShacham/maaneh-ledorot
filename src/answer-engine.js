...
const TOPICS={
 חינוך:['חינוך','ילד','ילדים','תלמיד','מורה','education','child','children','school','teacher','chinuch','קינדער','חינוך'],
 דאגה:['דאג','חרד','חרדה','פחד','בטחון','anxiety','worry','fear','bitachon','בטחון'],
 בריאות:['בריאות','רפואה','רופא','health','medical','doctor','refuah','רפואה'],
...
export function propositionRelevance(e,u){
 ...
 if(u.intent==='circumstances'&&text(e.documented_circumstances))score+=3;if(u.intent==='guidance'&&supportingProps.length)score+=3;if(u.intent==='historical'&&e.date)score+=2;if(u.intent==='bibliographic'&&(e.source_url||e.provenance))score+=2;
...
export function buildAnswerModel(index,q,{mode='regular'}={}){
 ...
 const cap=mode==='researcher'?8:4,selected=usable.slice(0,cap),claims=synthesizeClaims(selected,understanding,mode==='researcher'?16:6);
 ...
 if(mode==='researcher'){const witnessClasses=selected.reduce((a,e)=>(a[classifyWitness(e)]=(a[classifyWitness(e)]||0)+1,a),{});base.research={evidence_families:families.size,verified_sources:selected.filter(e=>e.status==='Verified').length,partial_context_sources:selected.filter(e=>e.context_status==='partial').length,periods:[...new Set(selected.map(e=>e.date).filter(Boolean))],audiences:[...new Set(selected.flatMap(e=>arr(e.audience)).filter(Boolean))],source_classes:[...new Set(selected.map(e=>e.source_class).filter(Boolean))],languages:[...new Set(selected.map(e=>e.language).filter(Boolean))],witness_classes:witnessClasses,pipeline_statuses:[...new Set(selected.map(e=>e.status).filter(Boolean))],witnesses:selected.map(e=>({family:familyKey(e),provenance:e.witness_provenance||e.source_witness||e.witness_type||null,verification_scope:e.verification_scope||null,context_status:e.context_status||'unknown',language:e.language||null,witness_class:classifyWitness(e),primary_secondary_status:e.primary_secondary_status||null,duplicate_or_parallel_relations:e.duplicate_or_parallel_relations||[]}))};}
 return base;
}
