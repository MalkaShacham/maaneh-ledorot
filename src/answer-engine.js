...
export function propositionRelevance(e,u){
  ...
  if(u.topics.length&&topicHit===0) return {score:0,support:'lexical_only',reasons:['topic absent from propositions']};
  ...
  let score=topicHit*2+rawHit*3+(e.status==='Verified'?2:0);
  if(u.intent==='circumstances'&&text(e.documented_circumstances)) score+=3;
  if(u.intent==='guidance'&&arr(e.response_propositions).length) score+=3;
  if(u.intent==='historical'&&e.date) score+=2;
  if(u.intent==='bibliographic'&&(e.source_url||e.provenance)) score+=2;
  return {score,support:score>=5?'direct_support':'contextual_support',reasons:[`topic=${topicHit}`,`specific=${rawHit}`]};
}

export function retrieveEvidence(index,q,{limit=12,threshold=5}={}){
  ...
  const seen=new Set(), hits=[]; for(const r of reranked){const k=familyKey(r.e);if(seen.has(k))continue;seen.add(k);hits.push({...r.e,_relevance:r.score,_support:r.support});if(hits.length>=limit)break;}
...
export function buildAnswerModel(index,q,{mode='regular'}={}){
  ...
  const usable=hits.filter(e=>arr(e.response_propositions).length);
  ...
  const claims=selected.flatMap((e,sourceIndex)=>arr(e.response_propositions).map(p=>({text:String(p),source_numbers:[sourceIndex+1],evidence_family_id:familyKey(e),verification_status:e.status,context_status:e.context_status}))).slice(0,mode==='researcher'?16:6);
